...
% Code to implement harmonic demodulation, pitch shifting F0 manipulation, and synthesis of speech signals.
...
% pitch scale modification factor (psm)
alpha = 0.75;
...
for i = 1:length(fname)
    ...
    % do pitch shifting
    [sub_sigs_new] = tvh_pitch_scale(sub_sigs,qhd,alpha);

    % do qhd synthesis
    [sub_sigs_new] = tvh_synthesis(sub_sigs_new,qhd,Fs);

    % do save files
    suffix = ['_psm_alpha_' strrep(num2str(alpha),'.','_')];
